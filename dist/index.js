...
let activebtns = [0, 0, 0]; // 2. Fetched array of data from Lectures.json

let lectures; // 3. array of the buttons
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isLecturePage) {
    ...
    fetch('lectures.json').then(result => {
      ...
      return result.json();
    }).then(data => buaTilFyrirlestur(data, fyrirlesturNumer)).catch(error => console.error(error));
  } else {
    ...
    fetch('lectures.json').then(result => {
      ...
      return result.json();
    }).then(data => getdata(data.lectures)).catch(error => console.error(error));
  }
});
...
function isrelevant(i) {
  ...
  if (totalActiveBtns === 1 || totalActiveBtns === 2) {
    for (let j = 0; j < 3; j++) if (lectures[i].category === category[j]) if (activebtns[j]) return 1;else return 0;
  } else return 1;
}
/*
...
function hreinsaSidu() {
  {
    var e = document.querySelector('.adal'); //e.firstElementChild can be used.
